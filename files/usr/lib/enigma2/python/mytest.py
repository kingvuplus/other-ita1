...
def runScreenTest():
	...
	if config.italysat.async_plug_load.value and os.path.isfile("/usr/lib/enigma2/python/Plugins/SystemPlugins/SpeedUp/plugin.pyo"):
		profile("loadBasePlugins")
...
profile("Init:OnlineSecurityCeckState")
from Plugins.SystemPlugins.Access.Test2 import *
italysecurityceckpoller.start()
...
profile("WeatherLoad")
import Components.Converter.YWeatherPoller
Components.Converter.YWeatherPoller.WeatherDownload()
...
from ItalySat.ItalysatUtils import OnDsl
if OnDsl():
	profile("IPTVEpgLameDB")
	if config.epg.iptv.value == True:
		try:
			import ItalySat.ItalysatIptvEpg
			ItalySat.ItalysatIptvEpg.ItalyLameDB()
			enigma.eDVBDB.getInstance().reloadBouquets()
		except:
			pass
	profile("IPTVEpgReference")
	if config.epg.iptvreference.value == True:
		try:
			print "Service IPTV"
			import ItalySat.ItalysatIptvEpg
			ItalySat.ItalysatIptvEpg.ReferenceIptv()
			enigma.eDVBDB.getInstance().reloadBouquets()
		except:
			pass
	profile("VODBuffer")
	if config.italysat.vodbuffer.value in ("1","2"):
		try:
			import ItalySat.ItalysatIptvDownload
			#from ItalySat.ItalysatIptvDownload import ItalyVODBuffer
			ItalySat.ItalysatIptvDownload.ItalyVODBuffer()
		except:
			pass
else:
	pass
